...
public class Map implements Event {
	...
	private void addEnemies(Game game){
		...
		Fighter mago= game.factory.generateMage(game.level);
		añadirCondimentos(mago,game);

		Fighter rogue= game.factory.generateRogue(game.level);
		añadirCondimentos(rogue,game);

		Fighter warrior= game.factory.generateWarrior(game.level);
	...
	private void añadirCondimentos(Fighter npc, Game game){
		...
		else if(npc.strategy.getStrategy()== "agresiva"){
			
			npc=new Poderoso(npc);
			
		}
		else if(npc.strategy.getStrategy()== "defensiva"){
